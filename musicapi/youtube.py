...
class YouTubeClient:
    def __init__(self) -> None:
        ...
        if settings.ytdlp_cookies_file:
            cookies_path = self._writable_cookiefile(Path(settings.ytdlp_cookies_file))
    ...
    def _writable_cookiefile(self, source: Path) -> Path:
        if not source.exists():
            return source
        target = Path(tempfile.gettempdir()) / "musicapi-youtube-cookies.txt"
        try:
            if not target.exists() or source.stat().st_mtime > target.stat().st_mtime:
                shutil.copyfile(source, target)
            return target
        except OSError as exc:
            LOGGER.warning("Could not copy cookie file to writable temp path: %s", exc)
            return source
